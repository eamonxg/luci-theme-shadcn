...
No linter CLI; the only tests are `.dev/tests/*.test.js` (plain `node:test`, no deps). Prettier (with `prettier-plugin-tailwindcss`) runs on format-on-save and sorts `@apply`/class lists — don't hand-reorder them.
...
`router-shadcn.js` (loaded from `footer.ut` after `menu-shadcn`, only when `window.navigation` exists) turns navigation between LuCI **view** pages into same-document swaps via the Navigation API; every other case — call/cbi/function pages, browsers without the API, a document poisoned by a foreign stylesheet, an expired session — stays a full load. **Read `.dev/docs/router.md` before touching navigation, teardown, page-scoped patches, `header.ut`'s `<head>`, or `#maincontent`'s scroll.** Template hooks it depends on: `data-shadcn-shell` on every stylesheet header.ut renders (a sheet without it or `data-shadcn-patch`/`data-shadcn-node-css` marks the document poisoned), `body[data-asset-version]` (`{# PKG_VERSION #}`, substituted by luci.mk so router-added links hit the same cache entry), `body[data-patches]`, `#maincontent[tabindex=-1]`; `menu-shadcn.js` must keep `syncRoute()`/`closeSurfaces()` working. Unlike aurora, `#maincontent` is the scroller here, so the router remembers its `scrollTop` per history entry itself. `pnpm test` covers the resolver and gates; the router's own budget is 15 KB built.
...
- `menu-shadcn.js`: resolves the `admin` branch of `ui.menu.load()`, then renders a two-level sidebar; `ICON_MAP` maps a LuCI menu node's `name` to `/shadcn/icons/*.svg`; deeper levels render as `#tabmenu`. `syncRoute()` re-marks the active sidebar item (same longest-link-prefix rule as the header.ut replay), breadcrumb and tabs from `L.env` after a router swap without rebuilding the sidebar; `closeSurfaces()` closes palette/drawer/popover
